layui.use(['element', 'form', 'layer', 'upload'], function () {
    ...
    /* 最大化窗口 */
    $(".alan-screen-full").on("click", function (e) {
        e.preventDefault();
    ...
    $(".ajax-status").on("click", function (e) {
        e.preventDefault();
        var checked = [];
        var tdcheckbox = $(".alan-table td .alan-checkbox :checkbox:checked");
    ...
    // 展示数据列表-多选框
    var thcheckbox = $(".alan-table th .alan-checkbox :checkbox");
    thcheckbox.on("change", function () {
        var tdcheckbox = $(".alan-table td .alan-checkbox :checkbox");
    ...
    // 检测列表数据是否为空
    var alanTable = $(".alan-table tbody");
    if (alanTable.length > 0) {
        var children = alanTable.children();
        if (children.length === 0) {
            var length = $(".alan-table thead th").length;
            var trNullInfo = "<tr><td class='alan-table-null' colspan='"
                + length + "'>没有找到匹配的记录</td></tr>";
            alanTable.append(trNullInfo);
        }
    }
    ...
    var paramSkip = function () {
        var getSearch = "";
        // 搜索框参数
        $('.alan-search-box [name]').each(function (key, val) {
    ...
    /* 展示列表数据搜索 */
    $(document).on("click", ".alan-search-btn", function () {
        paramSkip();
    });
    /* 改变显示页数 */
